# 测试testuser2用户登录和笔记API访问
$loginData = @{
    username = "testuser2"
    password = "test123"
} | ConvertTo-Json

Write-Host "=== 步骤1: testuser2登录获取JWT令牌 ==="
try {
    ...
    Write-Host "testuser2登录成功!"
    $token = $loginResponse.token
    Write-Host "获得JWT令牌: $($token.Substring(0, 50))..."
    
    Write-Host ""
    Write-Host "=== 步骤2: testuser2使用JWT令牌访问笔记API ==="
    ...
    Write-Host "testuser2笔记API访问成功!"
    Write-Host "笔记数据: $($notesResponse | ConvertTo-Json -Depth 3)"
    
} catch {
    Write-Host "testuser2操作失败!"
