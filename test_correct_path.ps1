# Test Fixed Path Mapping

Write-Host "=== Testing Fixed Path Mapping ==="
...
# Step 2: Test admin endpoint with fixed path (/api/admin/users)
Write-Host "`nStep 2: Test admin endpoint with fixed path"
try {
    $headers = @{
...
# Step 3: Test user endpoint with fixed path (/api/users/me)
Write-Host "`nStep 3: Test user endpoint with fixed path"
try {
    $headers = @{
        'Authorization' = "Bearer $token"
    }
    
    Write-Host "Sending request to: http://localhost:8082/api/users/me"
    Write-Host "Authorization header: Bearer $($token.Substring(0, 20))..."
    
    $response = Invoke-WebRequest -Uri "http://localhost:8082/api/users/me" -Method GET -Headers $headers -UseBasicParsing
